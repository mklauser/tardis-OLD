...
def read_radiative_rates(fname):
    try:
        h5_file = h5py.File(fname)
        radiative_rates = h5_file['radiative_rates']
        return radiative_rates
    except IOError, err:
        print(err.errno)
        print(err)
        logger.critical('Cannot import. Error opening the file to read radiative_rates')
        raise
...
class AtomData(object):
    ...
    def from_hdf5(cls, fname=None):
        ...
        if 'radiative_rates' in h5_datasets:
            radiative_rates_data = read_radiative_rates(fname)
        else:
            radiative_rates_data =None
        ...
        atom_data = cls(atom_data=atom_data, ionization_data=ionization_data, levels_data=levels_data,
                        ...
                        ion_cx_data=ion_cx_data,radiative_rates_data=radiative_rates_data)
        try:
    ...
    def __init__(self, atom_data, ionization_data, levels_data, lines_data, macro_atom_data=None, zeta_data=None,
                 collision_data=None, synpp_refs=None, ion_cx_data=None,radiative_rates_data=None ):
